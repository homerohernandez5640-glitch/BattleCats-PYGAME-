...
class newEnemy():
    ...
    def remove_catBase(self,BaseHealth):
        current_time = pygame.time.get_ticks()
        if self.type == "dog" and (current_time - self.last_hit_time > self.cooldown_period):
            self.last_hit_time = current_time  # Reset the cooldown timer
            BaseHealth -=30
        elif self.type == "snake" and (current_time - self.last_hit_time > self.cooldown_period):
            self.last_hit_time = current_time  # Reset the cooldown timer
            BaseHealth -=15
        return BaseHealth
